# FahamPesa Core User Modules

This document is the canonical reference for the user-side FahamPesa business modules, excluding the super-admin layer for managing all business accounts.
...
- Top section must include branch selector, search product, add product, bulk upload, export report.
- Table columns:
  - Product name
  - Barcode with label print
  - Category
  - Quantity
  - Reorder level
  - Cost price
...
- Product detail page shows product info, image, supplier, cost price, selling price, profit margin, stock per branch, and stock movement history.
- Stock increases by purchase, manual adjustment, transfer in.
- Stock decreases by sale, transfer out, adjustment.
- Every adjustment must save quantity, reason, user, and timestamp.

## 3. Supplier Module

- Manage suppliers and purchase tracking.
- Supplier list must show supplier name, contact, total purchases, outstanding balance, and status.
- Add supplier fields:
  - Name
  - Contact person
  - Phone
  ...
  - Opening balance
  - Payment terms
- Supplier detail page shows total purchases, total paid, and outstanding balance.
